...
// screen, validates them by calling GetApp, and persists the
...
func run(logPath string) error {
	var (
		...
		agentFlag   = flag.String("agent", "", "agent ID or name to bind to (auto-picks if exactly one agent in app; interactive picker otherwise)")
	)
	flag.Parse()
...
func acquireConfig(forceSetup bool) (*Config, *tavora.App, error) {
	if !forceSetup {
		if env := EnvConfig(); env != nil {
			if ws, err := validate(env); err == nil {
				slog.Info("connected via env credentials", "app", ws.Name)
				return env, ws, nil
			} else {
		...
		if cfg, err := LoadConfig(); err == nil {
			if ws, err := validate(cfg); err == nil {
				slog.Info("connected via stored credentials", "app", ws.Name)
				return cfg, ws, nil
			} else {
...
func validate(cfg *Config) (*tavora.App, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	return tavora.NewClient(cfg.URL, cfg.APIKey).GetApp(ctx)
}
